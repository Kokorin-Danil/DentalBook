...
<div class="container mt-4">
    <h1 class="text-center">Повторный визит</h1>
    ...
    <div class="card">
        <div class="card-header">
            <i class="bi bi-graph-up"></i> Динамика состояния
        </div>
        <div class="card-body">
            <textarea class="form-control" id="conditionDynamics" rows="4" placeholder="Опишите изменения в состоянии пациента с момента последнего визита"></textarea>
        </div>
    </div>
    ...
    <div class="card">
        <div class="card-header">
            <i class="bi bi-check-circle"></i> Результаты лечения
        </div>
        <div class="card-body">
            <textarea class="form-control" id="treatmentResults" rows="4" placeholder="Опишите результаты проведенного лечения"></textarea>
        </div>
    </div>
...
<script>
    ...
    document.getElementById('saveDataButton').addEventListener('click', async () => {
        ...
        const conditionDynamics = document.getElementById('conditionDynamics').value;
        const treatmentResults = document.getElementById('treatmentResults').value;

        // Проверка на заполненность данных
        if (!patientId || !conditionDynamics || !treatmentResults) {
            alert('Пожалуйста, заполните все поля!');
            return;
        }

        const data = {
            type: "Повторный визит",
            conditionDynamics: conditionDynamics,
        ...
        try {
            ...
            if (response.ok) {
                ...
                document.getElementById('conditionDynamics').value = '';
                document.getElementById('treatmentResults').value = '';
