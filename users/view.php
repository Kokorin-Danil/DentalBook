...
<body>
        ...
        <div class="table-container">
            ...
            <table class="table table-bordered">
                ...
                <tbody id="visitsTableBody">
                    <!-- Данные визитов загружаются динамически -->
                </tbody>
            </table>
        </div>
    </div>
    ...
    <script>
        // Функция изменения статуса визита
        async function changeVisitStatus(visitId, newStatus) {
            ...
            try {
                ...
                alert('Статус визита успешно изменен.');
                await loadVisits(); // Обновляем таблицу визитов
            } catch (error) {
                console.error('Ошибка изменения статуса визита:', error);
                alert('Не удалось изменить статус визита.');
            }
        }

        // Динамическое добавление кнопок для изменения статуса
        async function loadVisits() {
            ...
            tableBody.innerHTML = data.visits.map(visit => `
                <tr id="visit-${visit.id}">
                    ...
                    <td>
                        <button class="btn btn-sm btn-success" onclick="changeVisitStatus(${visit.id}, 'Подтвержден')">Подтвердить</button>
                        <button class="btn btn-sm btn-warning" onclick="changeVisitStatus(${visit.id}, 'Не подтвержден')">Снять подтверждение</button>
                        <button class="btn btn-sm btn-danger" onclick="changeVisitStatus(${visit.id}, 'Отменен')">Отменить</button>
                    </td>
                </tr>
            `).join('');
        }

        // Вызываем загрузку визитов при загрузке страницы
